...
\setmonofont{Ubuntu Mono} %等寬字體
\setCJKmainfont[AutoFakeBold=6]{Noto Serif CJK TC Medium}    %中文字型
\XeTeXlinebreaklocale "zh"						%中文自動換行
\XeTeXlinebreakskip = 0pt plus 1pt				%設定段落之間的距離
\setCJKmonofont[AutoFakeBold=6]{Noto Serif CJK TC Medium}    %中文字等寬字型
\setcounter{secnumdepth}{3}						%目錄顯示第三層

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand\digitstyle{\color{DarkOrchid3}}
\colorlet{ForestGreen4}{ForestGreen}
\makeatletter
...
breakautoindent=false,\t\t\t\t\t\t\t\t% disable automatic continuation indentation
postbreak=\mbox{\hspace{2em}},\t\t\t\t\t\t\t% indent wrapped lines like a tab
escapeinside={\%*}{*)},							% if you want to add a comment within your code
morekeywords={constexpr, ll},									% if you want to add more keywords to the set
morekeywords=[2]{vector,string,pair,tuple,queue,deque,set,map,multiset,multimap,unordered_map,unordered_set,priority_queue,function,optional,variant,bitset,array,stack,complex,initializer_list},
keywordstyle=\color{Blue1},
keywordstyle=[2]\color{ForestGreen4},
...
\newcommand{\inputcode}[2]{
  \subsection[#1]{#1 \footnotesize{[\texttt{\captureshell{md5sum "#2" | cut -c-6}}]}}
    \lstinputlisting{#2}
}
